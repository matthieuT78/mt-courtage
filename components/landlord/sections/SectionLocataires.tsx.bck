...
export function SectionLocataires({ userId, tenants, leases, properties, onRefresh }: Props) {
  ...
  /* ======================================================
     CRUD (ANTI-NAVIGATION + DEBUG RESULT)
  ====================================================== */
  ...
  const saveTenant = async () => {
    ...
    try {
      console.log("[saveTenant] start", { userId, selectedId, supabase: !!supabase, form });
      ...
      console.log("[saveTenant] payload", payload);
  ...
  const deleteTenant = async () => {
    ...
    if (hasAnyLeaseForTenant(selectedId)) {
      setErr("Suppression impossible : ce locataire est lié à un bail. Supprimez/terminez d’abord le bail associé.");
      return;
    }
    ...
    try {
      ...
      console.error("[SectionLocataires] delete error:", e);
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5 space-y-5">
      <SectionTitle
        ...
        desc="Créer, modifier, supprimer. Visualisez le bail actif et l’historique des baux."
      />
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <button
                ...
                onClick={(e) => {
                  stop(e);
                  if (loading) return;
                  console.log("[btn] saveTenant clicked", { selectedId });
                  saveTenant();
                }}
                disabled={loading}
