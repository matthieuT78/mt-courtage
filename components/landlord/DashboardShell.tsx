...
export function DashboardShell(props: any) {
  ...
  // ✅ logs visibles à chaque changement d’onglet
  useEffect(() => {
  ...
  const content = useMemo(() => {
    ...
    switch (active) {
      ...
      case "biens":
        return <SectionBiens userId={userId} properties={properties} photos={photos} onRefresh={props.refresh} />;

      case "locataires":
        return <SectionLocataires userId={userId} tenants={tenants} leases={leases} properties={properties} onRefresh={props.refresh} />;

      case "baux":
        return <SectionBaux userId={userId} leases={leases} properties={properties} tenants={tenants} onRefresh={props.refresh} />;

      case "loyers":
        return <SectionLoyers payments={payments} leases={leases} propertyById={props.propertyById} tenantById={props.tenantById} />;
      ...
      case "finance":
        return <SectionFinance leases={leases} payments={payments} propertyById={props.propertyById} />;

      case "etat_des_lieux":
        return <SectionEtatDesLieux userId={userId} leases={leases} properties={properties} tenants={tenants} onRefresh={props.refresh} />;
  ...
  }, [active, props, userId, userEmail, properties, tenants, leases, payments, receipts, photos]);

  return (
    <div
      className="max-w-7xl mx-auto px-4 py-6"
      // ✅ on garde un anti-submit global (utile), MAIS on ne casse pas les clics
      onSubmitCapture={(e) => {
        e.preventDefault();
        e.stopPropagation();
        console.warn("[DashboardShell] submit capturé/bloqué (anti reload).");
      }}
    >
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        <SidebarNav
          active={active}
          onChange={onChangeTab} // ✅ on passe la version loggée
