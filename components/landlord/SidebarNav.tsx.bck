...
export function SidebarNav({
  ...
  const go = (e: React.SyntheticEvent, key: LandlordSectionKey) => {
    // 🔒 empêche tout comportement “navigation / submit / bubbling”
    // et permet de diagnostiquer si le clic arrive bien ici
          ...
          {items.map((it) => {
            ...
            return (
              <div
                ...
                className={
                  "select-none cursor-pointer w-full text-left rounded-xl px-3 py-2 border transition flex items-center justify-between gap-2 " +
                  (isActive
        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <div
              ...
              className="cursor-pointer select-none rounded-full bg-slate-900 px-3 py-1.5 text-[0.75rem] font-semibold text-white hover:bg-slate-800"
            >
              + Bien
            </div>
            <div
