...
export default function ProjectsPanel() {
  ...
  const renderDetail = (project: ProjectRow) => {
    ...
    // PARC IMMOBILIER
    if (type === "parc") {
      const r = d.resumeGlobal || d.resume || {};
      const valeur =
        r.valeurParc ?? r.valeurTotale ?? r.valeurParcTotale ?? null;
      const cashflowMensuel =
        r.cashflowMensuelGlobal ??
        (typeof r.cashflowAnnuelGlobal === "number"
          ? r.cashflowAnnuelGlobal / 12
          : null);
      const rendement =
        r.rendementNetMoyen ?? r.rendementNetGlobal ?? r.rendementMoyen ?? null;
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-200 px-3 py-2.5">
              ...
              <p
                className={
                  "mt-1 text-sm font-semibold " +
                  (cashflowMensuel !== null && cashflowMensuel >= 0
                    ? "text-emerald-700"
                    : "text-red-600")
                }
              >
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500">
          Retrouvez ici vos simulations de capacité d&apos;emprunt, d&apos;investissement
          locatif, de parc immobilier ou de prêt relais. Vous pouvez consulter
          le détail, partager ou supprimer chaque projet.
        </p>
      </div>
      ...
      {!isLoading && error && (
        <div className="rounded-2xl border border-red-100 bg-red-50 shadow-sm p-4">
