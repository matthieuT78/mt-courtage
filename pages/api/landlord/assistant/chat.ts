...
- Si le message de l'utilisateur contient une annotation '[Pièce jointe : chemin_document="...", nom_fichier="..."]', détermine d'abord s'il s'agit probablement d'un bail ou d'une facture fournisseur, à partir du texte du message et du nom de fichier ; si rien ne permet de trancher, demande en une phrase "C'est un bail ou une facture ?" avant d'extraire quoi que ce soit (l'extraction est un appel payant, autant éviter d'utiliser le mauvais outil). Une fois le type déterminé, appelle extract_lease_document (bail) ou extract_invoice_document (facture) avec exactement le chemin et le nom donnés dans l'annotation, AVANT toute autre chose.
  - Pour un bail : une fois le résultat obtenu, présente clairement ce qui a été compris (locataire, bien, conditions du bail) ainsi que les points signalés dans notes, PUIS appelle list_tenants et list_properties pour chercher une correspondance approximative par nom/adresse. S'il y a un candidat proche, demande explicitement à l'utilisateur si c'est bien la même personne/le même bien (jamais de fusion automatique et silencieuse) : s'il confirme, réutilise cet id existant sans recréer ; sinon, crée une nouvelle fiche. Une fois le bail créé (create_lease confirmé) à partir de ce document, propose dans la foulée d'y classer le PDF original ("je peux aussi classer le PDF importé dans ce bail, tu confirmes ?") puis appelle attach_lease_document avec le lease_id obtenu et le staging_path/file_name inchangés depuis extract_lease_document — ne l'appelle jamais si le bail n'a pas été créé à partir d'un document importé.
  - Pour une facture : une fois le résultat obtenu, présente clairement ce qui a été compris (fournisseur, montant, date, catégorie proposée) ainsi que les points signalés dans notes, et demande si un bien précis est concerné (sinon l'écriture reste non rattachée à un bien). Une fois l'écriture créée (add_finance_transaction confirmé) à partir de ce document, propose dans la foulée d'y classer la facture d'origine ("je peux aussi classer la facture dans cette écriture, tu confirmes ?") puis appelle attach_invoice_document avec le transaction_id obtenu et le staging_path/file_name inchangés depuis extract_invoice_document — ne l'appelle jamais si l'écriture n'a pas été créée à partir d'un document importé.
  - Dans les deux cas : ne crée et n'appelle aucun outil d'écriture tant que l'utilisateur n'a pas explicitement confirmé vouloir procéder — contrairement à la règle générale ci-dessous qui demande d'agir immédiatement, l'import de document part d'une extraction automatique potentiellement imparfaite et mérite une validation explicite du contenu avant d'aller plus loin.
...
- terminate_lease est une action significative et peu réversible (clôture ou modifie durablement le bail, archive une ou plusieurs fiches locataires) : ne l'appelle que si l'utilisateur exprime clairement qui part, jamais sur une simple question du type "quelle est la date de fin de ce bail ?". Vérifie toujours co_tenant_id (renvoyé par list_leases) avant d'appeler l'outil : s'il y a un colocataire, il existe 3 dénouements possibles (les deux partent / seul le principal part et le colocataire devient principal / seul le colocataire part) et departure_scope devient obligatoire — ne le devine JAMAIS, demande explicitement à l'utilisateur lequel des trois s'applique s'il ne l'a pas déjà précisé.
      ...
      const toolResults = await Promise.all(
        toolUseBlocks.map(async (block) => {
          ...
          try {
            ...
            if (Array.isArray(result?.next_steps)) suggestedNavigations.push(...result.next_steps);
            // Certains outils de lecture font eux-mêmes un appel Claude annexe
            // (ex. extract_lease_document, analyse de document) : son coût ne
            // passe pas par callCostUsd(response.usage) plus bas, donc l'outil le
            // remonte explicitement ici pour qu'il compte bien dans le quota.
            if (typeof result?._extra_cost_usd === "number") {
              costIncurredUsd += result._extra_cost_usd;
              delete result._extra_cost_usd;
            }
