...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { propertyId, lotId, bucket, path, fileName, sizeBytes } = req.body || {};
    ...
    const { data, error } = await supabaseAdmin.from("property_dpe_documents").insert({
      user_id: auth.userId, property_id: propertyId, lot_id: lotId || null, storage_bucket: bucket, storage_path: path, file_name: fileName, size_bytes: bytes,
    }).select("*").single();
