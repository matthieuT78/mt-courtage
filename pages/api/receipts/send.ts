...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Json>) {
  ...
  const pdf = await buildPdfBuffer(async (doc) => {
    ...
    doc.fontSize(9).fillColor("#444").text(
      `Période : ${receipt.period_start} → ${receipt.period_end} • Émise le : ${receipt.issue_date}`,
      { align: "center" }
    );
    doc.moveDown(1);

    doc.fillColor("#111");
    doc.fontSize(10).text((receipt.content_text || "").trim() || "—", {
      align: "left",
    });
  ...
  // IMPORTANT: on garde la cohérence via (lease_id, period_start, period_end)
  ...
  if (payExisting.data) {
    ...
    if (insPay.error || !insPay.data) return res.status(500).json({ error: `Insert paiement échoué: ${insPay.error?.message}` });
  ...
  if (!emailResult.ok) {
    // On a quand même généré le PDF + marqué payé => à toi de décider si tu veux rollback.
    // Ici : on retourne une erreur UX claire.
